...
Page({
    ...
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          ...
          util.http({
            url: app.globalData.siteUrl + '/Main/Member/DeleteProListItem?devicetype=4&uid=' + wx.getStorageSync('SessionUserID'),
            method: "POST",
            data: {
  ...
  saveBill() {
    ...
    this.upLoadMp3(() => {
      util.http({
        url: app.globalData.siteUrl + '/Main/Member/InsertorUpdateProList?devicetype=4&uid=' + wx.getStorageSync('SessionUserID'),
        method: "POST",
        data: {
