...
Page({
  ...
  savehaibao: function (that) {
    ...
    if (ret.status == 1 && ret.success&&ret.Data.length>0){
      ...
      that.setData({
        lisdata: that.data.lisdata.concat(ret.Data),
      })
    } else if (ret.status == 0 && ret.success && ret.Data.length==0){
      if (currpage == 1) {
        that.setData({
          lisdata: '',
          shownodata: true
  ...
  onShareAppMessage: function () {
    ...
    return {
      ...
      path: 'pages/productdetail/productdetail?pid=' + that.data.lisdata[that.data.sharpindex].pid + '&ruid=' + ruid + '&rcompany=' + wx.getStorageSync('rcompany'),
