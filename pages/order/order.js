...
Page({
  ...
  toReceipt(e) {
    app.alerts("您确定已收到货物了吗？", {
      successBack: () => {
        util.http({
          url: app.globalData.siteUrl + '/Main/Member/ReceiptOrder?devicetype=4&uid=' + wx.getStorageSync('SessionUserID'),
          method: "POST",
          data: {
  ...
  toPay(e) {
    app.showLoading("请求中")
    util.http({
      url: app.globalData.siteUrl + '/Main/ShoppingSinaPay/GetPayOrderInfo?devicetype=4',
      data: {
        uid: wx.getStorageSync('SessionUserID'),
        orderid: e.currentTarget.dataset.orderid,
        wxapplet: '2',//小程序标识
        devicemac: wx.getStorageSync("SessionUserID"),
        payid: '20180929145425946136'
      },
      header: 1,
      successBack: (ret) => {
  ...
  cancleOrder(e) {
    app.alerts("确认关闭订单?", {
      successBack: () => {
        util.http({
          url: app.globalData.siteUrl + '/Main/Member/CloseOrder?devicetype=4&uid=' + wx.getStorageSync('SessionUserID'),
          method: "POST",
          data: {
            uid: wx.getStorageSync('SessionUserID'),
  ...
  getMsg(type) {
    ...
    util.http({
      url: app.globalData.siteUrl + '/Main/Member/GetOrderListJson?devicetype=4',
      data: {
        uid: wx.getStorageSync('SessionUserID'),
        currpage: this.data.page[this.data.activeTab],
        pagesize: this.data.pagesize,
