...
Page({
  ...
  onLoad: function (options) {
    ...
    var that = this
    utils.http({
      url: app.globalData.siteUrl + '/marketing/redpackage/GetActivitiesSharePro?devicetype=4',
      successBack: (ret) => {
        console.log(ret)
    ...
    setTimeout(function () {
      if (wx.getStorageSync('SessionUserID') != '') {
        wx.request({
          url: app.globalData.siteUrl + '/main/WechatApi/BinaryConversion',
          data: {
  ...
  sharequan: function () {
    ...
    utils.diz(that.data.listdata[that.data.clcikindex].pid, 10, 32, function (ret) {
      that.erweimaimg(ret)
    })
  },
  erweimaimg: function (ret) {
    ...
    erweima = app.globalData.siteUrl + '/Main/WechatApi/GetWxacodeun?scene=' + dizruid + ',' + id + ',' + wx.getStorageSync('rcompany') + '&page=pages/productdetail/productdetail&width=430&devicetype=4'
    that.erweimaon()
  },
  erweimaon: function () {
    var that = this
