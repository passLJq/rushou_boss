...
Page({
  ...
  tupian:function(){
    ...
    //     //画头像 商品头像
    //     ctx.drawImage(res.path, 300, 48, 144, 144)
    //     //最后绘画
  ...
  onShareAppMessage: function () {
    ...
    return {
      title: '卖货神器 天天卖爆货 速来！！',
      path: 'pages/yshopapply/yshopapply?ruid=' + ruid + '&rcompany=' + wx.getStorageSync('rcompany'),
      imageUrl: 'https://images.rushouvip.cn/IMG/csa1.jpg'
    }
  },
  //弹出分享框
  goshare: function () {
  ...
  sharequan: function () {
    ...
    //二维码地址
    var erweima = app.globalData.siteUrl + '/Main/WechatApi/GetWxacodeun?scene=' + ruid + ',' + wx.getStorageSync('rcompany')+'&page=pages/yshopapply/yshopapply&width=430&devicetype=4'
    wx.showLoading({
      title: '生成海报中...',
    })
    //清空海报
      ...
      success: function (res) {
        ...
        wx.getImageInfo({
          ...
          success: function (eweima) {
            //获取图片完成 开始画画
            ctx.drawImage('../../image/vv13.jpg', 0, 0, 375, 650)
            //画头像 商品头像
            that.circleImg(ctx, saveimg[0], 148, 60, 45)
            ctx.setFontSize(18)
            ctx.setTextAlign('center')
            ctx.fillText(that.data.userinfo.shopname, 191, 180)  
            //小程序二维码
            ctx.drawImage(eweima.path, 118, 322, 140, 140)
            //最后绘画
            ctx.draw(false, function (e) {
              wx.canvasToTempFilePath({
                x: 0,
