...
Page({
  ...
  showIndex(e) {
    ...
    util.http({
      url: app.globalData.siteUrl + '/Main/Member/UpdateProListItem?devicetype=4&uid=' + wx.getStorageSync('SessionUserID'),
      data: { 
  ...
  deleteBill(e) {
    wx.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          util.http({
            url: app.globalData.siteUrl + '/Main/Member/DeleteProList?devicetype=4',
  ...
  getMsg(loading_icon) {
    ...
    new Promise((resolve, reject) => {
      if (this.data.slide === 0) {
        util.http({
          url: app.globalData.siteUrl + '/Main/Main/GetYShopProductListJson?devicetype=4',
          data: {
