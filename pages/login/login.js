        ...
        if (res.authSetting['scope.userInfo']) {
          ...
          wx.getUserInfo({
            success: function (user) {
              wx.request({
                url: app.globalData.siteUrl + '/main/Login/RegisterByWeChat?devicetype=4',
                data: {
                  telphone: that.data.telphone,
                  ...
                  wxsessionkey: wx.getStorageSync('wxsessionkey'),
									inviterUserId: wx.getStorageSync("ruid")
                },
                method: 'POST',
                success: function (ret) {
                  wx.hideLoading()
                  ...
                  if (ret.success && ret.status == 1 && ret.data != '') {
                    ...
                    //解除绑定时预防没有这个参数 压缩uid 海报用
                    if (wx.getStorageSync('SessionUserID') != '') {
                      utils.diz(wx.getStorageSync('SessionUserID'), 10, 32, function (ret) {
