...
Page({
  ...
  onLoad: function(options) {
    ...
    //scene是二维码进来的,二维码的参数组成是（ruid,rcompanyid）
    ...
    if (options.ruid || options.scene) {
      //如果分享id等于自己的证明看自己的店
      if (options.ruid == wx.getStorageSync('SessionUserID') || alee[0] == wx.getStorageSync('SessionUserID')) {
        that.setData({
          dianstatus: 1,
          btntext: ''
        })
        ...
        if (options.scene) {
          ...
          if (alee[1] == 'qiandao') {
            ...
            wx.setStorageSync('rcompany', alee[1])
          }
        }
          ...
          successBack: (ret) => {
            ...
            if (ret && ret.data.success && ret.data.status == 1 && ret.data.Data.length > 0) {
              ...
              for (var i = 0; i < ret.data.Data.length; i++) {
                qindanimg.push('../../image/CombinedShape.png')
              }
    ...
    if (res.from === 'button') {
      ...
      if (that.data.sharptye == 1) {
        return {
          ...
          imageUrl: 'https://images.rushouvip.cn/IMG/sae.jpg'
        }
