...
function sharequan(that, sharptye, wheretype) {
  ...
  if (sharptye == 1) {
    //二维码地址
    var erweima = app.globalData.siteUrl + '/Main/WechatApi/GetWxacodeun?scene=' + ruid + ',2&page=pages/index/index&width=430&devicetype=4'
    console.log(erweima)
    //关闭分享框
    that.setData({
    ...
    //wheretype 1是index分享海报的  2是class分享海报的 3是prolist分享海报的
    if (wheretype == 1) {
      pids = that.data.yshopdata[that.data.sharpindex].pid
    } else if (wheretype == 2) {
      pids = that.data.listdata[that.data.sharpindex].pid
    } else if (wheretype == 3) {
      pids = that.data.lisdata[that.data.sharpindex].pid
    } else if (wheretype == 4) {
      pids = that.data.pid
    } else if (wheretype == 5) {
      pids = that.data.rushBuyMsg[that.data.slide][that.data.nowShareIdx].pid
    } else if (wheretype === 6) {
      pids = that.data.prolistid
    } else if (wheretype === 7) {//Class页面
      pids = wx.getStorageSync('rcompany') || 17
    }
    //压缩商品id
...
function erweimaon(num, that, wheretype) {
  ...
  if (wheretype == 1) {
    ...
    let billShareMsg = that.data.listdata
    pics = [billShareMsg[0].pic, billShareMsg[1] && billShareMsg[1].pic]
    names = [billShareMsg[0].name, billShareMsg[1] && billShareMsg[1].name]
    saleprices = [Number(billShareMsg[0].salePrice).toFixed(2), billShareMsg[1] && Number(billShareMsg[1].salePrice).toFixed(2)]
    marketprices = [Number(billShareMsg[0].marketPrice).toFixed(2), billShareMsg[1] && Number(billShareMsg[1].marketPrice).toFixed(2)] //市场价格
  }
  var erweima = app.globalData.siteUrl + '/Main/WechatApi/GetWxacodeun?scene=' + wx.getStorageSync('SessionUserID') + ',' + id + '&page=pages/productdetail/productdetail&width=430&devicetype=4'
  if (wheretype === 6) { //清单分享
    erweima = app.globalData.siteUrl + '/Main/WechatApi/GetWxacodeun?scene=' + (wx.getStorageSync('ruid') || wx.getStorageSync('SessionUserID')) + ',' + id + '&page=pages/product_bill_edit/product_bill_edit&width=430&devicetype=4'
  }
  if (wheretype === 7) {
    var erweima = app.globalData.siteUrl + '/Main/WechatApi/GetWxacodeun?scene=' + wx.getStorageSync('rcompany') + ',' + (wx.getStorageSync('ruid') || wx.getStorageSync('SessionUserID')) + '&page=pages/class/class&width=430&devicetype=4'
  }
  console.log(erweima)
  var saveimg = [] //下载下来的本地路径
  var protext = [] //商品文字
  ...
  const ctxs = wx.createCanvasContext('canvasCircle1')
  if (wheretype === 6 || wheretype === 7) {//清单分享
    //获取头像
    wx.getImageInfo({
      src: String(wheretype === 6 ? that.data.userinfo.shoplogo.replace('http://', 'https://') : that.data.companyjson.shoplogo.replace('http://', 'https://')),
      success: function (res) {
        console.log(res)
        saveimg.push(res.path)
        if (pics[0]) {
              ...
              if (pics[1]) {
                ...
                wx.getImageInfo({
                  ...
                  success: (billshareRet) => {
                    ...
                    billShareDraw(that, saveimg, erweima, ctxs, names, saleprices, marketprices,wheretype)
                  },
                  fail: (res) => {
                    wx.hideLoading()
                    app.alerts('商品图片获取失败')
...
function billShareDraw(that, saveimg, erweima, ctxs, names, saleprices, marketprices,wheretype) {
  //获取二维码
  wx.getImageInfo({
    src: String(erweima),
    success: function (eweima) {
      let billShareMsg = wheretype === 7 ? that.data.listdata : that.data.billList[0]
      let itemList = billShareMsg.itemlist || billShareMsg
      //获取图片完成 开始画画
      ctxs.setFillStyle('#fff')
      ctxs.fillRect(0, 0, 375, 667)
      //画头像 商品头像
      ctxs.drawImage(saveimg[0], 143, 25, 90, 90)
      // let wheretype = that.data.wheretype
      ...
      if (wheretype === 6) {
        ...
        ctxs.fillText(that.data.companyjson.shopname, 375 / 2, 140 + (wheretype === 7 ? 10 : 0))
      }
      if(wheretype === 6) {
        if (itemList.length && itemList.length > 1) {
          //第一个商品的信息
        ...
        ctxs.setFontSize(16)
        ctxs.setTextAlign('left')
        textfen(names[0], ctxs, 1)
        ctxs.setFontSize(18)
        ctxs.setTextAlign('left')
        ctxs.fillText('¥' + saleprices[0], 150, 315)
        ctxs.setFontSize(14)
        ctxs.setTextAlign('left')
        ctxs.setFillStyle('#999')
        ctxs.moveTo(290, 310)
        var width = 292 + ctxs.measureText('¥' + marketprices[0]).width
        ctxs.lineTo(width, 310)
        ctxs.stroke()
        ctxs.fillText('¥' + marketprices[0], 290, 315)
        if (itemList.length && itemList.length > 1) {
          //第二个商品的信息
          ctxs.setFillStyle('#000')
          ctxs.setFontSize(16)
          ctxs.setTextAlign('left')
          textfen(names[1], ctxs, 2)
          ctxs.setFontSize(18)
          ctxs.setTextAlign('left')
          ctxs.fillText('¥' + saleprices[1], 150, 440)
          ctxs.setFontSize(14)
          ctxs.setTextAlign('left')
          ctxs.setFillStyle('#999')
          ctxs.moveTo(290, 435)
          ctxs.setFillStyle('#999')
          var width = 292 + ctxs.measureText('¥' + marketprices[1]).width
          ctxs.lineTo(width, 435)
          ctxs.stroke()
          ctxs.fillText('¥' + marketprices[1], 290, 440)
        }

      }

      if (that.data.wheretype === 6) {
        //清单简介
        ctxs.setFontSize(18)
        ctxs.setTextAlign('center')
        ctxs.setFillStyle('#333')
